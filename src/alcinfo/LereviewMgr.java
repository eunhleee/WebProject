...
=======
	...
	public Vector<LereviewBean> getBoardList(int num,String keyField,String keyWord,int start,int cnt){
		...
		try {
			...
			if(keyWord.trim().equals("")||keyWord==null) {
				//�˻��� �ƴѰ��
				sql = "select num,lr_lnum,lr_nick,lr_id,lr_content,lr_ip,lr_star,lr_date,lr_count,lr_name "
						+ "from lereview "
...
>>>>>>> refs/remotes/origin/master
			...
			while(rs.next()) {
				...
				bean.setLr_date(rs.getString("lr_date"));
				bean.setLr_count(rs.getInt("lr_count"));
				bean.setLr_name(rs.getString("lr_name"));
	...
	public void insertLer(LereviewBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert lereview(lr_lnum,lr_name,lr_content,lr_ip,lr_star,"
					+ "lr_nick,lr_id,lr_date) "
					+ "values(?,?,?,?,?,?,?,now())";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, bean.getLr_lnum());
			pstmt.setString(2, bean.getLr_name());
	...
	public LereviewBean getReview(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select num,lr_lnum,lr_name,lr_content,lr_ip,lr_star,lr_nick,"
	...
	public void updateLer(LereviewBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "update lereview set lr_name=?, lr_star=?, lr_content=? where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getLr_name());
